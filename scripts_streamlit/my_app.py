...
# def predict_image(img_array):
#     # img_array est déjà en RGB
#     img = np.array(Image.fromarray(img_array).resize((IMG_SIZE, IMG_SIZE)))
#     img = img.astype(np.float32) / 255.0
#     img = np.expand_dims(img, axis=0)
...
# st.markdown("""
# Téléchargez une image ou prenez une photo pour détecter les fruits.  
# Le systeme s'ocupe d'analyser l'image d'abord, puis de traiter la maturité de chaque fruit détecté.
# """)
...
#     # Lecture image avec PIL pour éviter libGL
